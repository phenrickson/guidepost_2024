...
list(
  ...
  tar_target(
    season_games_to_date,
    season_game_info |>
    filter(completed == T) |>
    prepare_game_info() |>
    add_game_weeks() |>
    prepare_fcs_teams() |>
    join_team_estimates(estimates = season_team_estimates)
  ),
  tar_target(
    season_games_upcoming,
    season_game_info |>
    add_game_weeks() |>
    filter(completed != T) |>
    prepare_game_info() |>
    ...
    inner_join(
      season_team_estimates |>
      find_most_recent_team_estimates(game_info = season_game_info) |>
      select(
        home = team,
        home_overall = postgame_overall,
    ...
    inner_join(
      ...
      select(
        away = team,
        away_overall = postgame_overall,
        away_offense = postgame_offense,
        away_defense = postgame_defense,
        away_special = postgame_special
      )
    )
  ),
  tar_target(
  ...
  tar_target(
    ...
    left_join(
      season_game_info |>
      add_game_outcomes() |>
      select(game_id, week, home_points, away_points, home_margin, home_win, total_points)
    ) |>
    select(season, season_type, week, everything())
  ),
  tar_target(
    games_predictions,
    games_sims |>
